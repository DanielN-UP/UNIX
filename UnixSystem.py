...
def viewStudents(): # view students
    ...
    for person in students.each():
        keyuserName.append(int(person.key()))
    return(keyuserName)
...
def displayloans(keyuserName,uISBN):
    print("Displaying books out on loan")
    stringhead =("{:<30} {:<30} {:<30} {:<30} {:<30}".format('userName','Email','ISBN','Title','Date'))
...
def displaybooks(keyuserName):#allows the books to be viewed in a table
    stringhead =("{:<10} {:<30} {:<30} {:<30} {:<30}".format('ISBN', 'Title', 'Author', 'Category', 'Year'))
...
def mybooks(userName): #views my books
    try:
        ...
        return "No books out on loan"

def isAdmin(gmail):
    if(gmail.lower() == "adminilibrary@gmail.com"):
        return True
    else:
        return False
